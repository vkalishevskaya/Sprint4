package praktikum.pages;

public class EnvConfigs {
    public static final String INVALID_ORDER_NUBMER = "123";
    public static final int DEFAULT_TIMEOUT = 10;
    static final String BASE_URL = "https://qa-scooter.praktikum-services.ru/";
    public static final String DEFAULT_NAME = "Иван";
    public static final String DEFAULT_SECONDNAME = "Иванов";
    public static final String DEFAULT_ADRESS = "Иваново";
    public static final String DEFAULT_PHONE_NUMBER = "+1234567890";
    public static final String FUTURE_DATE = "01.01.2025";
    public static final String PHONE_NUMBER_WITHOUT_PLUS = "1234567890000";
    public static final String EXP_DATE = "01.01.2023";
}
